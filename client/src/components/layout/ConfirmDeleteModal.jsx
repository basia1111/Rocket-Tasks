  ...
  return (
    <AnimatePresence>
      {isModalActive && (
        ...
        >
          <motion.div
            className="sm:min-w-[500px] p-4 flex flex-col items-center rounded-xl shadow-lg bg-white text-center"
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            exit={{ scale: 0 }}
            transition={{
          ...
          >
            ...
            <div className="text-lg pb-4">{message}</div>
            <button
              onClick={() => (handleDelete(id), setIsModalActive((m) => false))}
              className="text-base rounded-full px-2 py-1 bg-red-600 text-white"
            >
              Confirm
            </button>
          </motion.div>
